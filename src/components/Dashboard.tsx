...
import { formatRupiah, formatDateIndonesian, formatNumber } from '../utils/formatters';
import { 
  TrendingUp, 
  TrendingDown, 
  Wallet, 
  ...
  return (
    <div id="dashboard-container" className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 sm:gap-4 relative z-10">
          <div>
            ...
            <p className="hidden sm:block text-sm text-teal-100/90 mt-1.5 max-w-2xl font-light">
              {isBendahara 
                ? 'Kelola penerimaan iuran 50 sekolah anggota MKKS Citos dan alokasi pengeluaran operasional secara transparan.'
                : 'Pantau riwayat pembayaran iuran sekolah Anda dan laporan keuangan MKKS Citos secara real-time.'}
            </p>
          </div>
      ...
      {!isBendahara && mySekolahObj && (
        <div className="bg-gradient-to-r from-emerald-50 via-teal-50 to-indigo-50 border border-teal-200 rounded-2xl p-4 sm:p-6 shadow-sm">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 sm:gap-4">
            ...
            <div className="flex items-center justify-between sm:justify-end space-x-3 pt-2 sm:pt-0 border-t sm:border-t-0 border-teal-200/60">
              ...
              <button
                ...
                className="bg-teal-600 hover:bg-teal-700 text-white font-medium text-xs px-3.5 py-2 sm:px-4 sm:py-2.5 rounded-xl shadow-md transition-all flex items-center space-x-1.5 shrink-0"
              >
          ...
          {/* Month progress pills for this school */}
          <div className="mt-3.5 pt-3.5 border-t border-teal-200/60">
            <span className="text-xs font-semibold text-slate-700 mb-2 block">Status Pembayaran Iuran Bulan (100rb / Bulan):</span>
              ...
              {BULAN_LIST.map((bulan, idx) => {
                ...
                return (
                  <div
                    ...
                    className={`text-center py-2 px-1.5 rounded-xl text-xs font-bold border transition-all ${
                      isPaid
                        ? 'bg-emerald-600 text-white border-emerald-700 shadow-xs'
                        : 'bg-white text-slate-400 border-slate-200'
                    }`}
                  >
                    <div className="text-[10px] sm:text-[11px] uppercase tracking-wide">{BULAN_SINGKAT[idx]}</div>
                    <div className="text-[9px] sm:text-[10px] mt-0.5">{isPaid ? 'LUNAS' : 'BELUM'}</div>
                  </div>
                );
              })}
            </div>
      ...
      {/* Financial Trend Chart & Quick Actions */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        
        {/* Visual Trend Bar Chart (2 Columns) */}
        <div className="lg:col-span-2 bg-white rounded-2xl p-4 sm:p-6 border border-slate-200 shadow-sm flex flex-col justify-between">
          <div>
        ...
        {/* Quick Overview Per School Status (Bendahara) / Quick Shortcuts */}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center space-x-2 shrink-0">
                      ...
                      {isBendahara && !isFullyPaid && onSelectSchoolForIuran && (
                        <button
                          ...
                          className="bg-teal-600 hover:bg-teal-700 text-white text-[10px] px-2 py-1 sm:px-2.5 sm:py-1.5 rounded-lg font-semibold shadow-xs transition-all shrink-0"
          ...
          {isBendahara && (
            <div className="mt-4 pt-3.5 border-t border-slate-100">
              <button
                ...
                className="w-full bg-teal-600 hover:bg-teal-700 text-white font-semibold text-xs py-2.5 rounded-xl shadow-md transition-all flex items-center justify-center space-x-2"
              >
      ...
      <div className="bg-white rounded-2xl p-4 sm:p-6 border border-slate-200 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2">
          ...
          <button
            ...
            onClick={() => onNavigateToTab('laporan-keuangan')}
            className="text-teal-600 hover:text-teal-700 text-xs font-semibold hover:underline self-start sm:self-auto flex items-center space-x-1"
          >
